...
nicfg.tcp.behaviorcycle = 60; % Train cycle in seconds. E.g., 30 means 30 seconds from start to start. Default 30.
...
nicfg.scoptophotometry.trainlength = 200; % Opto pulses per train. E.g., 10 means 10 pulses per train. Default 10.
...
nicfg.scheduler.delay = 300; % Delayed opto start in seconds. E.g., 120 means 2 min delay. Default 120s.
...
nicfg.optodelayTTL.delay = 200; % Delay in 100 ms. E.g., 20 means 2 seconds. Default 20 (2s)
...
nicfg.optodelayTTL.lead = 20; % How many seconds is the food TTL armed before an opto train. This does not affect the cue/actoin/reward delays above. Default 4s. Max 65535s.
